...
export default function RetromanPage() {
  ...
  useEffect(() => {
    ...
    imageRafRef.current = requestAnimationFrame(() => {
      const result = applyRetroman(imageData, paramsRef.current);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const result = applyRetroman(frame, paramsRef.current);
      const canvas = canvasRef.current!;
  ...
  const exportWebM = (secs: number, fromStart = false) => {
    ...
    const doRecord = () => {
      ...
      recorder.onstop = () => {
        ...
        const a = document.createElement('a');
        a.href = url; a.download = 'retroman.webm'; a.click();
      ...
      <div style={{ width: 360, minWidth: 360, background: '#1a1a1a', borderRight: '1px solid #222', overflowY: 'auto', overflowX: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '16px 18px 14px' }}>
          ...
          <p style={{ fontSize: 12, color: '#888', lineHeight: 1.65, margin: '0 0 14px' }}>
            Two-color dithering engine. Reduces the image to ink on a colored background using classic halftone dither algorithms.
          </p>
        ...
        <div style={{ padding: '0 18px 48px', display: 'flex', flexDirection: 'column' }}>
          ...
          {/* Algorithm */}
          <Sect label="Algorithm" />
          <p style={hint}>Atkinson = classic Mac look. Floyd = maximum detail. Bayer = ordered matrix. Blue = smooth organic grain.</p>
          <BtnGrid
            options={ALGORITHMS}
            value={params.algorithm}
            cols={4}
            onChange={(v) => set('algorithm')(v.toLowerCase() as RetromanParams['algorithm'])}
          />
          ...
          <Slider label="Contrast"   value={params.contrast}   min={-100} max={100} step={1} onChange={set('contrast')} />

          {/* Resolution */}
          <Sect label="Resolution" />
          <p style={hint}>Pixelizes before dithering. Higher values create chunky retro pixel art.</p>
          <Slider label="Scale" value={params.scale} min={1} max={8} step={1} unit="x" onChange={set('scale')} />

          {/* Ink color */}
          <Sect label="Color" />
          <p style={hint}>Ink / foreground — the dark pixels in the dither pattern.</p>
          <ColorPicker colors={FG_COLORS} selected={params.fgColor} onChange={set('fgColor')} />

          {/* Background color */}
          <Sect label="Color 2" />
          <p style={hint}>Background — the light pixels in the dither pattern.</p>
  ...
  return (
    ...
    >
      {!hasMedia && (
        ...
        >
          <div style={{ fontFamily: '"Courier New", monospace', fontSize: 52, fontWeight: 900, letterSpacing: '0.1em', marginBottom: 12 }}>RETROMAN</div>
