...
import { renderRetroman, RetromanParams } from '@/lib/effects/retroman';
...
const DEFAULT_PARAMS: RetromanParams = {
...
export default function RetromanPage() {
  ...
  const [params, setParams] = useState<RetromanParams>(DEFAULT_PARAMS);
  ...
  useEffect(() => {
    ...
    imageRafRef.current = requestAnimationFrame(() => {
      if (canvasRef.current) renderRetroman(canvasRef.current, imageData, params);
    });
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      renderRetroman(canvasRef.current, vCtx.getImageData(0, 0, w, h), paramsRef.current);
    };
    ...
    if (file.type.startsWith('image/')) {
      ...
      vid.onloadeddata = () => {
        if (vid.duration > 300) {
          ...
          setVideoError('Video must be under 5 minutes.'); return;
        }
  ...
  const handleSave = () => {
    ...
    const ok = saveCanvasToGallery(canvas, 'Retroman', 'retroman', mediaType === 'video' ? 'video' : 'image');
    if (ok) { setSavedFeedback(true); setTimeout(() => setSavedFeedback(false), 1800); }
  };

  const set = <K extends keyof RetromanParams>(key: K) =>
    (val: RetromanParams[K]) => setParams((p) => ({ ...p, [key]: val }));
      ...
      <div style={{ width: 320, minWidth: 320, background: C.surface, borderRight: `1px solid ${C.border}`, overflowY: 'auto', overflowX: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '16px 18px 14px' }}>
          ...
          <p style={{ fontSize: 12, color: C.textDim, lineHeight: 1.65, margin: '0 0 14px' }}>
            Two-color dithering engine. Stacks Bayer / Blue Noise kernels with pixel-scale control.
          </p>
        ...
        <div style={{ padding: '0 18px 48px', display: 'flex', flexDirection: 'column' }}>
          <Sect label="Dither Mode" />
          <BtnGrid options={DITHER_MODES} value={params.mode} cols={2}
            onChange={(v) => set('mode')(v.toLowerCase().replace(' ', '-') as RetromanParams['mode'])} />

          <Sect label="Grid" />
          <Slider label="Pixel Size" value={params.pixelSize} min={1} max={16} step={1} onChange={set('pixelSize')} />
          <Slider label="Threshold"  value={params.threshold} min={0} max={255} step={1} onChange={set('threshold')} />

          <Sect label="Tone" />
          <Slider label="Contrast"   value={params.contrast}   min={0.5} max={3}  step={0.05} onChange={set('contrast')} />
          <Slider label="Brightness" value={params.brightness} min={-100} max={100} step={1} onChange={set('brightness')} />
          <Slider label="Grain"      value={params.noise}      min={0}   max={0.5} step={0.01} onChange={set('noise')} />
          <Slider label="Sharpen"    value={params.edgeEnhance} min={0}   max={1}   step={0.05} onChange={set('edgeEnhance')} />

          <Sect label="Color" />
          <ColorPicker colors={COLORS} selected={params.fgColor} onChange={set('fgColor')} />
          <div style={{ height: 8 }} />
          <ColorPicker colors={COLORS} selected={params.bgColor} onChange={set('bgColor')} />

          <Sect label="Advanced" />
          <Toggle label="Serpentine Scanning" value={params.serpentine} onChange={set('serpentine')} />
        </div>
      </div>
...
function BtnGrid({ options, value, cols, onChange }: {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: 5, marginBottom: 8 }}>
      {options.map((opt) => {
        const active = value.toLowerCase() === opt.toLowerCase().replace(' ', '-');
        return (
